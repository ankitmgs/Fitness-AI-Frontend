...
const CaloriesBurnedChart: React.FC<CaloriesBurnedChartProps> = ({ data }) => {
  ...
  const dailyData = data.reduce((acc, log) => {
    acc[log.date] = (acc[log.date] || 0) + log.caloriesBurned;
    return acc;
  ...
  if (chartData.length < 2) {
    return (
        <p className="text-center text-gray-500 dark:text-gray-400 py-8">
          Log workouts on at least two different days in this period to see a chart.
        </p>
    );
  }
  ...
  const valueRange = maxValue === 0 ? 100 : maxValue;
